...
| `agents/context-agent.md` | M | Phase A 6 提醒 + Phase F 私库注入 | 完整，但内含**硬编码"本作（<example-project>）"** |
...
| 10 | 新建 `/tmp/dummy-project` 跑 private-csv | 0 | **+0/total 89** | **重大 bug**：dummy 项目未提供任何数据，但写到与<example-project>同一份 CSV（共享 fork 目录） |
...
**实测**：在新建 dummy-project 跑 `private-csv --table ai-replacement-vocab --chapters 1-5`，CLI 直接对**<example-project>的 89 条共享 CSV** 操作（exit=0）。如果 dummy 项目有数据可提取，会**追加到<example-project>的 CSV**。

context-agent.md line 714 明确读 `${CLAUDE_PLUGIN_ROOT}/references/private-csv/ai-replacement-vocab.csv`（fork 共享路径）。

**后果**：用户开新项目"<example-project-D>"，跑一次 private-csv 后：
- <example-project> 89 条本作专属反例（含"<protagonist>/麦穗/<example-project> Ch4 末"等本作人物/情节锚点）会注入"<example-project-D>" 的 writing_guidance.local_blacklist
- "<example-project-D>"提取的反例会污染<example-project>的 CSV
- 两本书互相干扰

README.md line 7 自称"sync-cache 后所有项目自动受益"——**与本作专属内容矛盾**：跨项目自动注入"<protagonist>专属：拧手表/咬笔帽"会让其他项目角色变得奇怪。

### 4.2 anti-ai-guide.md 5 类本作根因边界标注（P2）

anti-ai-guide.md line 88 表头明标"**本作（<example-project>）专属 5 类根因映射**"，N1-N5 列出"半度/半秒/半指/半分"、"<protagonist>特有的微动作（咬笔帽/拧手表/数手指）"等。

边界标注**正确**——N1-N5 表本身有"本作（<example-project>）专属"标题。但是 Step 2A 加载该文件时，跨项目 writer 会把"<protagonist>拧手表"当作通用规则，可能**轻度误导**。

修复建议：N1-N5 段加 frontmatter 字段 `applies_to: ["<example-project>"]`，跨项目按当前 project_id 自动跳过这一段。

### 4.3 anti-ai-guide.md Phase X1 自检 5 项跨项目通用 (OK)

Phase X1 5 项前置自检（金手指首披露 / 突兀编号 / 大纲爽点兑现 / 伏笔铺设 / 读者卡点）通用，不绑定本作。

### 4.4 context-agent.md 硬编码"本作（<example-project>）" (P1)

line 723: "同样读 canon-violation-traps.csv，取**本作（<example-project>）**相关前 5 条到 writing_guidance.canon_traps"。

**这是字面文本"本作（<example-project>）"硬编码进 prompt**。跨项目用户开新书时，context-agent 会按字面"<example-project>"过滤 canon_traps（找不到匹配 → 退化为不注入）。
...
| **P1-2** | context-agent.md 硬编码"本作（<example-project>）" | 跨项目用户开新书时，canon_traps 注入因字面字符串不匹配而退化 |
...
迁移：把现有 4 张 CSV 移动到 `references/private-csv/<example-project>/`，fork 共享目录留空或仅放跨作品通用的 AI 癖好（第二本小说开新目录）。
...
```diff
- 同样读 canon-violation-traps.csv，取本作（<example-project>）相关前 5 条
...
| 5 | P1-2 | context-agent.md 字面"本作（<example-project>）"改为按 state.project.id 动态过滤 | 30 min |
