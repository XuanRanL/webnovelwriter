...
**Tech Stack:** Python 3.11；fork ↔ plugin cache 双向同步（`webnovel.py sync-cache`）；既有 hygiene_check 24+ H 项；13 checker（含 reader-naturalness / reader-pull / prose-quality / consistency）已部署；<example-project>项目 11 章 polish_reports + naturalness/consistency/pacing/emotion 等 13 类 tmp JSON 已积累作为私库素材；state_manager.py 已有 `update --set-checker-score / --set-chapter-meta-field / --append-recheck / --sync-protagonist-display` 子命令架构可直接扩展。
...
2. **每个改 fork 文件的 Phase 必跑**：
   ...
   python -X utf8 <example-project>/.webnovel/hygiene_check.py
   ```
   三条 exit=0 才能 commit
...
<example-project>（路径中含中文，bash 注意引号）作为基线项目。Ch1-11 已成稿；Ch12 起所有新机制必须在该项目跑出兑现指标。
...
- <example-project>项目 .webnovel/tmp/ 中 checker JSON 命名有两套历史格式：`{checker}_check_ch{NNNN}.json`（多数）和 `{checker}_ch{NNNN}.json`（Ch9）—— Phase F extractor 必须两种都扫
...
**Files:**
- Create: `<example-project>/.webnovel/reports/quality_baseline.json`

- [ ] **Step 1: 跑提取脚本**

```bash
mkdir -p "<example-project>/.webnovel/reports"
python -X utf8 - <<'PY'
import json
from pathlib import Path
proj = Path("<example-project>")
...
```bash
ls -la "<example-project>/.webnovel/reports/quality_baseline.json"
python -X utf8 -c "import json; d=json.load(open('<example-project>/.webnovel/reports/quality_baseline.json',encoding='utf-8')); print(json.dumps(d['avg_ch1_11'],ensure_ascii=False,indent=2))"
```
...
git add docs/superpowers/plans/upstream-snapshot/COMMIT_REF.txt \
        ...
        "<example-project>/.webnovel/reports/quality_baseline.json"
...
python -X utf8 -c "
import json
s=json.loads(open('<example-project>/.webnovel/state.json',encoding='utf-8').read())
...
```bash
grep -E "anti-ai-guide|起草前预防|Anti-AI" <example-project>/.webnovel/context/ch0012_context.json 2>/dev/null | head
```
...
```bash
grep -E "first-chapter\|Round 19.*A.*首句钩" <example-project>/.webnovel/tmp/reader_pull_ch0012.json 2>/dev/null
```
...
python -X utf8 -c "
import json
p='<example-project>/.webnovel/tmp/naturalness_check_ch0012.json'
try:
...
用法:
    python private_csv_extractor.py --project <example-project> \
...
3. 把每条的"坏样本→好样本"作为本章起草禁词清单写入 `writing_guidance.local_blacklist`：
   ...
   ```
4. 同样读 `canon-violation-traps.csv`，取本作（<example-project>）相关的前 5 条到 `writing_guidance.canon_traps`：
   ```json
   {
     "canon_traps": [
...
- 条目数 ≥ 10
- "大模型指令"具体可执行
- "毒点"列对本作（<example-project> · 科幻末世）适用
- "示例片段"是干货
...
> 数据来源：`<example-project>/.webnovel/reports/quality_baseline.json`
> 报告时间：（写报告的真实日期）
...
```bash
git add "<example-project>/.webnovel/reports/quality_baseline.json" \
...
| feedback_ch5_<example-project>_root_causes | hygiene check / state update CLI 保留 |
