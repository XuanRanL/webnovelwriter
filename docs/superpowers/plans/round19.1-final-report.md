...
- Round 19 Phase F 已往 fork 共享目录写了 89+28+16+3 = 136 条**<example-project>本作专属反例**（含"<protagonist>/麦穗/印记/半度/半秒"等本作私有标记）
- 用户开新项目（如《<example-project-D>》/《<example-project-B>》）时，writer 起草前会 Read 这 136 条<example-project>反例 → **跨项目误导写作**
...
4. <example-project> 4 张表 140 条数据迁移到 `<example-project>/.webnovel/private-csv/`
...
| <example-project>本地 4 表行数 | 90+29+17+4=140 | ✅ 140 行 |
...
| 5 | <example-project>本地 CSV 含数据 | 140 条 | ✅ 90+29+17+4=140 |
...
| 11 | F private-csv 转发项目本地 | total 89（<example-project>本地） | ✅ +0/total 89 |
...
| 10 | `<example-project>/.webnovel/private-csv/*.csv` × 4 | NEW · 项目本地（被 gitignore，不入主仓） | +140 行 |
...
- ✅ Round 19.1 P0-1 后**新项目（《<example-project-D>》/新作）开干 Round 19 全部 9 个 Phase 自动生效**
- ✅ 私库每项目专属，不污染
...
- [x] P0×3 全部根治
- [x] 12 项端到端测试全过
- [x] <example-project>本作数据从 fork 共享迁移到项目本地（gitignore，不会推到 GitHub）
...
- P1-2 context-agent.md 硬编码 "本作（<example-project>）" 字面（影响新项目阅读）
