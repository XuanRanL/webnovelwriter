...
- [ ] **Step 2: 跑<example-project>项目 hygiene**
...
- preflight exit=0
- hygiene_check <example-project>项目 exit=0
...
python -X utf8 -c "
import sqlite3, sys
conn = sqlite3.connect('<example-project>/.webnovel/index.db')
...
Expected: 两表都 EXISTS。
- 若 `state_changes` MISSING → 需先在 index_manager 加表（Phase 3.5 处理，但<example-project>项目应已有）
...
kq = KnowledgeQuery(Path('<example-project>'))
...
```bash
python -X utf8 webnovel-writer/scripts/data_modules/webnovel.py --project-root <example-project> knowledge query-entity-state --entity 林夏 --at-chapter 5
```
...
**目的**：upstream 的 `genre-canonical.md` 定义了 15 个 canonical_genre + 37 个 platform_tag 的两层映射。本地 `state.json.project_info.genre` 各项目自由填写（"<example-project>"/"末世"/"科幻末世"），导致 CSV 检索无法精准过滤。统一成 canonical_genre 后：
...
- [ ] **Step 3: 给<example-project>项目 backfill canonical_genre**
...
p = Path('<example-project>/.webnovel/state.json')
...
Expected: 输出 `canonical_genre = 科幻`（"<example-project>" → 科幻末世 → canonical 科幻）。
