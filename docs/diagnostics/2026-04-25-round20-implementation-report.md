...
#### 3.3 总纲三计划 schema ✓ (`<example-project>/大纲/总纲.md:161-260`)
...
- Ch13 锁死："<golden-finger-space>种出第一批可食用作物（标题"种出"首次兑现）"
...
| `<example-project>/大纲/总纲.md` | EDIT | +99 | 低（项目数据） |
...
import json
s=json.load(open('<example-project>/.webnovel/state.json',encoding='utf-8'))
...
### 写 Ch13 时
- Ch13 必须命中：1 次 medium 金手指（<golden-finger-space>种出第一批可食用作物）+ 1 次 D 类冲突（陆老师摊牌）+ 1 次决策钩
...
modified:   <example-project>/.webnovel/state.json  (Ch12 hook_close 回填)
modified:   <example-project>/大纲/总纲.md          (+99 line: 三计划)
