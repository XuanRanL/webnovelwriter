...
### <example-project>历史 visual 缺失章节

- Ch4 / Ch8 / Ch9 嗅觉零（汽修厂 / 城市傍晚 / <golden-finger-space>空间二度入境）
...
| # | 文件 | 改动 | 行数 |
|---|------|------|------|
| 1 | `skills/webnovel-write/references/first-chapter-hook-rubric.md` | NEW · A/B/C 三项追读契约 + Ch2/Ch3 跨章弱版 + <example-project> Ch1 复盘 | ~95 |
...
### <example-project> Ch1 复盘验证

<example-project> Ch1（“我又活了”）当前 v3 polish 后 reader-pull=93 / reader-critic=91，A/B/C 三项已自然满足。本 rubric 主要为未来重写 Ch1 或新书首章兜底。
...
| P1-11 | chapter_meta.naturalness_verdict 缺失，data-agent 经常漏写 | data-agent.md L572 有规范但落实不到位 | 项目本地 hygiene_check.py naturalness_log_check 加 auto-sync：从 .webnovel/tmp/{checker}_recheck/check_ch{NNNN}.json 读 verdict + score 自动写入 | `<example-project>/.webnovel/hygiene_check.py:128+` | ✅ 根治 |
...
**现象**：<example-project> Ch1-Ch4 每次 commit 后，`state.progress.total_words` 都被覆盖成本章字数，而非所有章累加
...
- 项目《<example-project>》ch0004_prep.md 修正后：0 漂移
...
| P1-8 | 低 | <example-project> Ch1 checker_scores 只有 10/13（缺 flow/naturalness/reader-critic） | 历史遗留（Round 12/13 v2 之前数据） | 下章 trend 失真 |
...
preflight 实战验证（<example-project>项目）：
```
OK polish_drift: i:\...\<example-project>\正文
```
...
| 项目侧 · 配置 | `.webnovel/plan_consistency_config.json` | <example-project>项目的规则集 · 13 条 drift + B1 gender + 密度 tracks |
...
1. **webnovel-init 层未加防御** — 虽然 write 层加了 naturalness-veto，但新项目 init 时 AI 可能**再次自编伪神经科学**（《<example-project>》项目开篇策略.md L193 "4 字激活杏仁核 0.3 秒"就是 init 时 AI 编的，不是 plugin 钦定）。根因不在 plugin 模板，而在 init 流程没有"反伪科学"硬约束提示 AI。
...
Ch1 v1 首句"<protagonist>在死。"是汉语语病（"在死"违反现代汉语体貌），19 个审查器（10 内部 + 9 外部）+ Step 6 七层审计**0 抓**，用户一眼看出。根因 6 层：
...
| 测试 | v1 首句"<protagonist>在死。" | v2 首句"<protagonist>第七次拨通..." |
|---|---|---|
...
1. **开篇策略模板**删除所有"字数阈值"伪科学话术（当前只在《<example-project>》项目开篇策略.md 局部修了，其他项目 init 时仍可能被污染）
...
**详细报告**：`<example-project>/.webnovel/tmp/flow_test/ABC_FULL_DEPLOYMENT_REPORT.md`（~400 行完整分析）
...
- <example-project>本地 shim 和 webnovel-init SKILL 的 shim 模板同步更新

**验证**：
- 单元 test 扩展为 16 个 case，全部通过
- <example-project> Ch1/Ch2 hygiene_check 17/17 pass
...
| 9 | hygiene_check.py 只在<example-project>项目本地，每个新项目都得手写 | 1) 抽进 `webnovel-writer/scripts/hygiene_check.py` 作为框架资产，含 17 项检查（H1-H17）；2) webnovel-init SKILL 在收尾阶段自动部署 shim 到 `.webnovel/hygiene_check.py`；3) 项目本地扩展可用 `.webnovel/hygiene_check_local.py` 定义 `run(root, chapter, report)`；4) <example-project>本地 shim 已改为委托框架版 | `scripts/hygiene_check.py`（新）, `skills/webnovel-init/SKILL.md`, `<example-project>/.webnovel/hygiene_check.py`, `<example-project>/.webnovel/hygiene_check_local.py` |
...
### <example-project>项目数据修复（Phase 2）
...
| 9 框架化 hygiene | 横向收益：将来每个新项目都有同样的 commit 前防御；<example-project>的 H13（力量体系版本）继续作为本地扩展 |
...
**新增文件（<example-project-B>项目级）**：
| 文件 | 说明 |
|------|------|
| `<example-project-B>/设定集/典故引用库.md` | 7 类约 40 条引用总库 + 密度规则 + 第 1 卷 14 处引用规划表 |
| `<example-project-B>/设定集/原创诗词口诀.md` | 4 组世界内原创口诀（空亡者古谣/<example-project-B>铭文/六甲空亡歌完整版/"算过了"进化谱系）|

**修改文件（<example-project-B>项目级）**：
| 文件 | 修改内容 |
|------|---------|
| `<example-project-B>/设定集/伏笔追踪.md` | 新增"典故伏笔"分类（9 条，M02/M03/M04/M05/D01/D04/O01/O02/O03）|
| `<example-project-B>/大纲/第1卷-详细大纲.md` | Ch4/Ch5/Ch9/Ch10/Ch15/Ch37/Ch44/Ch50 新增"引用锚点"字段 |
...
| 文件 | 修改内容 |
|------|---------|
| `<example-project-B>/审查报告/第0003章审查报告.md` | 从 JSON 重建 9 模型 × 10 维度矩阵 |
| `<example-project-B>/.webnovel/state.json` | 删除 "1"/"2" 重复键 |
| `<example-project-B>/.webnovel/tmp/review_metrics.json` | "人物OOC"→"人物塑造" + notes 更新 |
| `<example-project-B>/.webnovel/index.db` | review_metrics 重新落库 |
...
| 文件 | 修改内容 |
|------|---------|
| `<example-project-B>/.webnovel/state.json` | `chapter_meta["0003"]` 从 15 字段重写为 30 字段完整结构 |
...
**目标**：让 Round 20.x 所有改动从"<example-project>项目特有"真正变成"任何新书都自动获益的全流程标准"。
