...
def test_outline_has_three_release_plans():
    """Round 20 · 批3-B：<example-project> 项目 总纲.md 必须含 3 个 release plan section."""
    # parents[4] = repo root I:\AI-extention\webnovel-writer
    project_root = Path(__file__).resolve().parents[4] / "<example-project>"
    if not project_root.exists():
        pytest.skip(f"<example-project> 项目不存在 ({project_root})")
