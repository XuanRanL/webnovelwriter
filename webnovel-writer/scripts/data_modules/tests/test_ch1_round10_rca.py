...
# -*- coding: utf-8 -*-
"""
Ch1 <example-project> Round 10 RCA 回归测试 · 2026-04-16
...
def test_verify_quote_exact_match():
    ...
    text = "<protagonist>第七次拨通苏灵号码的时候，正跪在合肥地铁二号线大东门站的月台边上。"
    assert er._verify_quote_exists("<protagonist>第七次拨通苏灵号码的时候", text) is True
...
def test_verify_quote_whitespace_normalized():
    er = _load_external_review()
    text = "<protagonist>   第七次拨通苏灵号码"
    assert er._verify_quote_exists("<protagonist>第七次拨通苏灵号码", text) is True
...
def test_verify_quote_long_quote_core_substring_fallback():
    er = _load_external_review()
    text = "<protagonist>第七次拨通苏灵号码的时候，正跪在合肥地铁二号线大东门站的月台边上"
    # long_quote 前 10 字 = "<protagonist>第七次拨通苏灵号" 吻合 text → 核心 10 字子串回退匹配
    long_quote = "<protagonist>第七次拨通苏灵号 还带了点尾巴与原文略微不同"
...
def test_hallucinated_quote_flow_end_to_end():
    ...
    er = _load_external_review()
    chapter_text = "<protagonist>第七次拨通苏灵号码的时候，正跪在合肥地铁二号线大东门站的月台边上。"
    # 两个 issue：一个真实 quote，一个幻觉 quote
    real_issue = {
        "id": "C1",
        "severity": "high",
        "quote": "<protagonist>第七次拨通苏灵号码",
    }
    halluc_issue = {
        "id": "H1",
        "severity": "critical",
