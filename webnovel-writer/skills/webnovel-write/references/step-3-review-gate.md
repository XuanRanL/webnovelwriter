...
**硬规则：5+5 分批启动，禁止 10 个 checker 同时并发。**

原因：Claude Code Agent 并发池限制约 4-6 个，10 个同时启动会导致排队超时（全部 0 bytes output 直到轮询窗口耗尽）。
...
2. Batch 1 全部返回后，启动 Batch 2 的 5 个 checker，同样等待全部返回。
