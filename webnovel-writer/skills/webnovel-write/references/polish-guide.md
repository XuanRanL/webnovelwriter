...
### 2.0c.bis canon-grep：polish 禁止凭印象造角色状态 / 位置 / 关系（Round 28.46 加入 · 防 R28.36 v5 + R28.46 #1 复发）

**为什么必要：**

R28.36 v5 教训（Ch45 postmortem）：第一次 postmortem 凭印象造非 Canon 简称"合肥物科院"传染 12 处，与 Canon §239 真源"中科院合肥物质科学研究院 / 中科院合肥所"硬冲突。

R28.46 #1 教训（Ch46 polish）：起草 + polish 时凭印象写"陆灵今晚在内院？" "在。她跟外公在洗野菜" + 主角内心"陆灵那个学校就在那一带" — 但 Canon §08-SSOT 一.D-7 明文锁定 "Ch21 陆灵移居改为'未移居'"，陆灵仍在合工大宿舍未入基地。幸好作者写完立即 grep Canon 自查到，改回短信形态（陆灵未出场，只在周明屋短信 callback）才未提交。

两次同源根因：**polish / 起草 阶段新增有名角色行为前，没 grep Canon 真源**。

**正确做法（硬规则）：**

polish / 起草时任何**新增有名角色行为段**（"角色 X 在 Y 地点 / 与 Z 交互 / 做 W 动作"），**必须**先做下列 4 步 grep 自检：

```bash
# 1. 角色当前位置（最关键）
grep -n "<角色名>" 设定集/00-Canon-Bible.md
grep -n "<角色名>" 设定集/08-连续性锁死表.md  # 或类似 SSOT 文件
grep -n "<角色名>" 设定集/03-角色口径表.md

# 2. 角色当前状态（觉醒/未觉醒 / 已入基地/未入 / 觉醒等级）
grep -nE "<角色名>.*(觉醒|印记|入基地|移居|住在|工作|学校)" 设定集/

# 3. 与目标地点/角色的关系
grep -nE "<角色名>.*<另一角色名>" 设定集/ 大纲/

# 4. 最近章节正文中该角色出现的实际位置
grep -nE "<角色名>" 正文/第00{N-3}章*.md  # 最近 3 章
```

**结果判定：**

- 4 步 grep 中有任一处明确锁定该角色的"未在 Y" / "未做 W" → **禁止** polish 中加 "X 在 Y" / "X 做 W"
- 4 步 grep 中只有间接证据（如"卷二才发力" / "Ch25-27 到基地" 等时间表） → polish 必须**遵循表中时序**，不能跳进度
- 4 步 grep 中找不到任何证据（即新行为）→ 这不是 polish，是新设定，**先进 canon 再进正文**

**与 §2.0c canon-check（专有名词）的关系：**

| 检查 | §2.0c | §2.0c.bis |
|---|---|---|
| 检测对象 | 物理专有名词（X 作物 / X 容器 / X 物理机制）| 角色行为/状态/位置/关系 |
| 触发场景 | polish 凭空发明物（金银花/灶屋木匣 类）| polish 凭印象写角色行为（陆灵今晚在内院 类）|
| 检查工具 | hygiene H67（自动）| 人工 grep（强制）|
| Round | 28.3 / 28.21 | **28.46（新加入）** |

**实操：每次 polish 涉及有名角色（非临时配角）的新增段落前，AI 必须在 chat 中先打出 grep 命令再 polish。** 这是非常硬的纪律，比 "我记得 Canon 是这样" 强 10 倍。

**已实战验证：** Ch46 polish 时 AI 凭印象写陆灵入内院，自查 grep 后发现 SSOT 一.D-7 明文锁"未移居"，立即改成短信 callback，避免了 R28.36 v5 级别的 Canon 传染。

---

### 2.0c.tri physical-state-trace：polish 新增物件位置描述前必 grep 同物件（Round 28.46 加入 · 防 L341 折角广告纸位置矛盾）

**为什么必要：**

Ch46 Step 4 polish 章末新加一段："桌上那张折角广告纸的折角，陆沉用拇指碾了碾..." 但 L341 章末紧接的前一段仍是 "他从兜里把折角广告纸再摸出来" — 物件"广告纸"瞬间从"兜里手上"跳到"桌上"，无空间过渡 → flow recheck 报 high "物理位置失控"。

**正确做法（硬规则）：**

polish 新增任何**物件位置描述**（"X 在桌上 / 兜里 / 手上 / 怀里 / 抽屉里"），必须先 grep 同物件在前后段落的最近一次位置描述：

```bash
# 1. 在本章正文 grep 同物件，确认前一次位置
grep -nE "(折角广告纸|那张广告纸|广告纸)" 正文/第00{N}章*.md
# 看 polish 新增段位置与最近一次位置之间是否有"位移动作"（摸出来/放下/搁下/塞回）

# 2. 在前一章正文确认物件原始位置（若有跨章携带）
grep -nE "(折角广告纸)" 正文/第00{N-1}章*.md
```

**判定：**

- 同物件前一次位置 = "兜里 / 手上" 且新增段写 "桌上 / 抽屉里" → **必须**在两段之间补一个"位移动作"（"他从兜里摸出来搁在膝盖上摊开" / "放到桌上"）
- 同物件位置跨段不一致 → 修法：要么补位移动作，要么改新增段位置（"兜里那张" 代 "桌上那张"）

**适用范围**（常见触发点）：
- 主角随身物：折角广告纸 / 沙漏 / 手机 / 钱包 / 钥匙 / 兜里那张纸
- 桌面 / 容器内物：碗 / 杯 / 笔 / 本子 / 钥匙
- 衣物状态：袖口 / 衣襟 / 兜口

**实操：每次 polish 涉及随身物 / 容器内物的新增段，AI 必须在 chat 中先打出 grep 命令再写。**

**已实战验证：** Ch46 Step 4.5 flow recheck 报 L341 位置矛盾 → 我立刻 Edit L341 改"摸出来搁在膝盖上摊开" + 章末段重写为"折角广告纸" 整段统一在膝盖上 → 物理状态链一致。
...
**触发条件**：
- 术语首次出现（全书/全卷第一次）
- 术语非常识（例：“阳眼”/“第三十八任”/“未闭之言”/“<example-project>未尽”/“<relic-item>” 非常识；“走/镜子/账册” 是常识字不需额外解释）
