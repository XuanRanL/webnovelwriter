...
| A3 | 外部模型覆盖 | 读 `.webnovel/tmp/external_review_*_ch{NNNN}.json`，统计有效模型数（路由 verified + 无 phantom_zero + 维度完整） | **Round 16/25 · 扁平共识**：≥ 10/15 有效 pass · 8-9 medium warn · 5-7 high warn（仍不阻塞） · <5 critical fail | **high**（仅 <5/15 时才 critical） | 重跑 Step 3.5（`--model-key all`）· 若 provider outage 可接受 |
