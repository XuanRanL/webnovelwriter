...
> **适用场景**：你已经用本 fork 写过《<example-project>》，现在要开新项目（如《<example-project-D>》/《<example-project>》/《<example-project-B>》或全新作品），需要让 Round 19 的所有质量护栏在新项目自动生效。

> **Round 19.1 P0-1 关键变更**：私库 CSV 改为**项目本地** `{project}/.webnovel/private-csv/`，跨项目隔离 — 不会再被《<example-project>》的"<protagonist>/麦穗/印记"反例污染。
...
```bash
# 假设新项目 "<example-project-D>" 已经跑完 Ch1-2 完整 Step 0-7
PROJ="<example-project-D>"
...
**禁止**把《<example-project>》的 `private-csv/*.csv` 拷到《<example-project-D>》下！<example-project>反例（"半度/半秒/<protagonist>/麦穗/印记"）对其他题材是**误导**。每个项目的私库必须从该项目自己的 `tmp/*.json` checker 数据派生。
...
## 3. 已存在项目（如《<example-project-B>》/《<example-project-D>》）补 Round 19
...
如果担心历史章节有 reader-critic <75 谷底（如《<example-project>》Ch3=62/Ch4=58），跑：
...
```bash
cp <example-project>/.webnovel/hygiene_check.py "${PROJ}/.webnovel/hygiene_check.py"
```
...
每个项目可以有自己的 `ROOT_CAUSE_GUARD_RAILS.md` 记录该项目特有 RCA。**<example-project>的 ROOT_CAUSE_GUARD_RAILS.md 不要复制到新项目**（项目特定根因）。
