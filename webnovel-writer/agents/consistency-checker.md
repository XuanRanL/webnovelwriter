...
**示例**（Ch7 应抓到的漂移）：
```
❌ 设定：设定集/金手指设计.md §2 <golden-finger-space>空间=随身维度+掌心印记激活
   → 正文：L247 "车库后门穿进地下通道·磨过的石板·沉下一寸"
   → VIOLATION: RANDOM_ACCESS 空间被写成 FIXED_ENTRY
   → severity=critical · 修法：改"按了一下掌心印记·<golden-finger-space>那头的晨雾接住了他"
```
...
- 校验规则：
  ...
  4. 对每处“<power-faction>/系统/印记相关实体 + 前世”共现做 C/C'/C'' 分类：
...
- 错误示例（Ch1 <example-project> qwen-plus critical）：
  ```
  ❌ [critical] 设定："<power-faction>印记是死亡瞬间被铜面具老者激活的金手指"
