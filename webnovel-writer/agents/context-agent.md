...
- 终检清单（本章必须满足项 + fail 条件）— **Round 28.46 加入硬项：标题词逐字显形 ≥1（Round 28.25 F7 镜像）**

### final_check_list 必含硬项（Round 28.46 加入）

**血教训**（Ch46 R28.46 #5）：Step 1 final_check_list 32 项**漏掉了**"标题逐字显形 ≥1"硬项 → Step 2A writer 起草时不知此约束 → Step 4 polish 完成后 Step 6 audit F7 fail high → 主流程 Step 6 阶段才匆忙补一句"林晚秋蹲下，把竹篮里的野菜束摆在青砖上"。

**永久规则**：final_check_list 必须包含下列**核心硬项**（项目无关，所有书通用）：

```json
{
  "final_check_list": [
    "字数 ∈ [hard_min, hard_max]（SSOT: state.word_count_policy）",
    "AI 陈词 10 词 = 0（像/仿佛/犹如/似乎/像是/好像/如同/一般/似的/宛如）",
    "中文弯引号 100% · ASCII 引号 = 0",
    "无 markdown 标题/分隔线/加粗（首行直接叙事）",
    "前世/今生/这一世 = 0（H77 P0 闸门）",
    "元标识符 第N章/Ch{N}/裸 Ch 数字 = 0（H40 P0 闸门）",
    "标题词逐字显形 ≥ 1 次（Round 28.25 F7 永久规则 · Round 28.46 镜像到 Step 1）",
    "角色称谓 SSOT 合规（如朵朵叫陆伯伯 / 苏瑾叫陆先生 等）",
    "Canon 简称合规（禁非 Canon 简称 · Round 28.36 v5）",
    "本章 ≥ N 个微兑现（来自 micro_payoffs_required）",
    "章末 ≥ 5 个未闭合问题（来自 unclosed_questions_count）"
  ]
}
```

**自检触发条件**：Step 1 context-agent 落盘前必须**遍历自检** final_check_list 数组，检查上述 11 项是否全部覆盖（不必逐字相同，但语义必须包含）。缺任一项 → 落盘前补上。

**特别说明 · 标题词显形**：
- 提取 `chapter_title` 中的核心词（如 "野菜束摆在青砖上" → 全句 / 或拆为 "野菜束" + "青砖上" 任一即可）
- 要求 Step 2A 起草时让该词逐字出现在正文 ≥1 次（自然融入，非生硬重复）
- Round 28.25 永久规则：F7 audit 严格匹配 `chapter_title` 整串字符串
...
- **红线7：认知路径缺失**（开篇章/重生章/穿越章必须在执行包内显式列出本章主角首次获得的关键情报及**载体-段号**锚点；例：`{情报:末世爆发倒计时, 载体:铜面具档案灌注, beat:Beat2}`）
...
4. 角色专属微动作（<protagonist>咬笔帽=焦虑、拧手表=不耐烦、数手指=思考），禁止全员“瞳孔微缩”
