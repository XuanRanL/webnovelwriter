...
> **【H71 P0 硬规则 · Round 28.20 第 5 次复发根治】** 详见 `${CLAUDE_PLUGIN_ROOT}/references/checker-output-schema.md` § H71 防御规则。
>
...
- **首句是决定性的**：如果首句汉语不通（如“<protagonist>在死。”这种“在+瞬时动词”机翻感），直接 `REJECT_CRITICAL + block_commit=true`
...
**与 Ch11 方言血教训的关系**：本子维度的“全员书面语”判定**必须先读** 项目级 `references/03-角色口径表.md` / `07-本地化资料包.md` 等设定集中的方言白名单；命中白名单的方言词不算违例。
