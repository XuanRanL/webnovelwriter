...
## Round 29 Phase 8 · 克制型角色情感判据（所有项目通用）

- **克制 = 外冷 + 漏一道缝，不是无情绪**。克制型主角（voice canon 标注寡言/冷静/收敛类）在**情感顶点场景**（至亲伤亡/重逢/背叛/告别等)必须有 ≥1 处情绪外漏的破绽——声音哑半拍/一个出格的小动作/一句没说完的话/一次违反自己习惯的行为。全场景零外漏（全部反应是程序化物理动作）→ issue (severity=high, id=EMO_SUPPRESSED_FLAT)。
- 判据是语境的，不是数量配额：非顶点场景的克制完全合法；顶点场景"指节收半寸+站半分钟"式纯外化也算零外漏（52 章实测读者反馈"50 行后开始怀疑这个人是不是没情绪"）。
- 该判据优先级高于签名密度顾虑：宁可让破绽动作重复，不可让顶点场景无情绪。
